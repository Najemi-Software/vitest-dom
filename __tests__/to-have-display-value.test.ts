...
describe("with multiple select", () => {
    ...
    it("matches only when all the multiple selected values are equal to all the expected values", () => {
        ...
        expect(subject.queryByTestId("select")).toHaveDisplayValue(["Ananas", "Banana"]);
    });

    it("matches even when the expected values are unordered", () => {
        const subject = mount();
        expect(subject.queryByTestId("select")).toHaveDisplayValue(["Avocado", "Ananas"]);
    });

    it("matches with regex expected values", () => {
        const subject = mount();
        expect(subject.queryByTestId("select")).toHaveDisplayValue([/[Aa]nanas/, "Avocado"]);
    });
});
